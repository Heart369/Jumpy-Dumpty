...
public class Zsdata {
    ...
    public List<Jswqdata> xmzs(){
        data=new ArrayList<>();
        String[] data1=new String[]{"Wanderer","Nilou","Collei","Dehya"};
        String[] data2= new String[]{"流浪者","妮露","科莱","迪希亚"};
    ...
    public List<Scdata> zysc(){
        sc = new ArrayList<>();
        String[] data=new String[]{"自由的哲学","繁荣的哲学","浮世的哲学","净言的哲学"};
        int[] data1 = new int[]{R.drawable.md_zy,R.drawable.ly_ft,R.drawable.jp_fs,R.drawable.xm_jy};
        for(int i=0;i<4;i++){
    ...
    public List<Scdata> zesc(){
        sc = new ArrayList<>();
        String[] data=new String[]{"抗争的哲学","勤劳的哲学","风雅的哲学","巧思的哲学"};
        int[] data1 = new int[]{R.drawable.md_kz,R.drawable.ly_ql,R.drawable.jp_fr,R.drawable.xm_dx};
        for(int i=0;i<4;i++){
            s1 = new Scdata(data1[i],data[i]);
            sc.add(s1);
        }
    ...
    public List<Scdata> zssc(){
        sc = new ArrayList<>();
        String[] data=new String[]{"诗文的哲学","黄金的哲学","天光的哲学","笃行的哲学"};
        int[] data1 = new int[]{R.drawable.md_sw,R.drawable.ly_hj,R.drawable.dq_tg,R.drawable.xm_ddx};
        for(int i=0;i<4;i++){
            s1 = new Scdata(data1[i],data[i]);
            sc.add(s1);
        }
    ...
    public  List<Scdata> zmsc(){
        sc = new ArrayList<>();
        String[] data=new String[]{"自由的哲学","抗争的哲学","诗文的哲学","繁荣的哲学","勤劳的哲学","黄金的哲学","浮世的哲学","风雅的哲学","天光的哲学","净言的哲学","巧思的哲学","笃行的哲学"};
        int[] data1 = new int[]{R.drawable.md_zy,R.drawable.md_kz,R.drawable.md_sw,R.drawable.ly_ft,R.drawable.ly_ql,R.drawable.ly_hj,R.drawable.jp_fs,R.drawable.jp_fr,R.drawable.dq_tg,R.drawable.xm_jy,R.drawable.xm_dx,R.drawable.xm_ddx};
        for(int i=0;i<12;i++){
            s1 = new Scdata(data1[i],data[i]);
            sc.add(s1);
        }
    ...
    public List<Jswqdata> wqxmze(){
        data=new ArrayList<>();
        String[] data1=new String[]{"_Catalyst_Ayus_Awaken","_Pole_Deshret_Awaken","_Catalyst_Pleroma_Awaken","_Catalyst_Arakalari_Awaken","_Pole_Arakalari_Awaken"};
        String[] data2= new String[]{"千夜浮梦","赤沙之杖","流浪的晚星","盈满之实","贯月矢"};
        int[] start=new int[]{5,5,4,4,4};
        for(int i=0;i<5;i++){
            Jswqdata js=new Jswqdata(data1[i],data2[i],start[i],0);
            data.add(js);
    ...
    public List<Jswqdata> wqxmzs(){
        data=new ArrayList<>();
        String[] data1=new String[]{"_Bow_Ayus_Awaken","_Catalyst_Alaya_Awaken","_Bow_Arakalari_Awaken","_Bow_Fin_Awaken","_Claymore_Pleroma_Awaken","_Claymore_Deshret_Awaken"};
        String[] data2= new String[]{"猎人之径","图莱杜拉的回忆","王下近侍","竭泽","玛海菈的水色","苇海信标"};
        int[] start=new int[]{5,5,4,4,4};
        for(int i=0;i<5;i++){
            Jswqdata js=new Jswqdata(data1[i],data2[i],start[i],0);
            data.add(js);
    ...
    public List<Scdata> wqzysc(){
        sc = new ArrayList<>();
        String[] data=new String[]{"高塔孤王的碎梦","孤光寒林的神体","远海夷地的金枝","谧林涓露的金符"};
        int[] data1 = new int[]{R.drawable.md_sm,R.drawable.ly_st,R.drawable.jp_jz,R.drawable.xm_jf};
        for(int i=0;i<4;i++){
            s1 = new Scdata(data1[i],data[i]);
            sc.add(s1);
        }
    ...
    public List<Scdata> wqzesc(){
        sc = new ArrayList<>();
        String[] data=new String[]{"凛风奔狼的怀乡","雾海云间的转还","鸣神御灵的勇武","绿洲花园的真谛"};
        int[] data1 = new int[]{R.drawable.md_hx,R.drawable.ly_zh,R.drawable.dq_yw,R.drawable.xm_zd};
        for(int i=0;i<4;i++){
            s1 = new Scdata(data1[i],data[i]);
            sc.add(s1);
        }
    ...
    public List<Scdata> wqzssc(){
        sc = new ArrayList<>();
        String[] data=new String[]{"狮牙斗士的理想","漆黑陨铁的一块","今昔剧画之鬼人","烈日权威的旧日"};
        int[] data1 = new int[]{R.drawable.md_lx,R.drawable.ly_yt,R.drawable.dq_gr,R.drawable.xm_lr};
        for(int i=0;i<4;i++){
            s1 = new Scdata(data1[i],data[i]);
            sc.add(s1);
        }
    ...
    public List<Scdata> wqzmsc(){
        sc = new ArrayList<>();
        String[] data=new String[]{"高塔孤王的碎梦","凛风奔狼的怀乡","狮牙斗士的理想","孤光寒林的神体","雾海云间的转还","漆黑陨铁的一块","远海夷地的金枝","鸣神御灵的勇武","今昔剧画之鬼人","谧林涓露的金符","绿洲花园的真谛","烈日权威的旧日"};
        int[] data1 = new int[]{R.drawable.md_sm,R.drawable.md_hx,R.drawable.md_lx,R.drawable.ly_st,R.drawable.ly_zh,R.drawable.ly_yt,R.drawable.jp_jz,R.drawable.dq_yw,R.drawable.dq_gr,R.drawable.xm_jf,R.drawable.xm_zd,R.drawable.xm_lr};
        for(int i=0;i<12;i++){
            s1 = new Scdata(data1[i],data[i]);
            sc.add(s1);
        }
